...
include("../scripts/shorefor_utils.jl")
include("../scripts/shorefor_functions.jl")
include("../scripts/create_shorefor_ind.jl")
include("../scripts/evaluation_metrics.jl")
include("../scripts/dataloader_monthly.jl")
...
function inds_to_preds(inds, datasets)
    preds = []
    for ds in datasets
        push!(preds, [])

        inputs = inputs_from_dataset(ds)
        for ind in inds
            CartesianGeneticProgramming.reset!(ind)
            dxdts = process(ind, inputs)[1]
            push!(preds[end], dxdts)
        end
    end
    return preds
end

function evaluate_dataset(ind::CGPInd, dataset; eval_start_index=1, eval_end_index=nothing)
    CartesianGeneticProgramming.reset!(ind)
    Tp, Hsb, Dir, dates_waves, loc_shore, t_shore, E, Sla, rivdis, omega, P,  t, phi = dataset
    dt_waves = dates_waves[2] - dates_waves[1]
    phi = ceil(phi / dt_waves)
...
function evaluate_individual(ind)
    score_grandpop = evaluate_dataset(ind, dataset_grandpopo_calib)
    score_narrabeen = evaluate_dataset(ind, dataset_narrabeen_calib)
    score_duck = evaluate_dataset(ind, dataset_duck_calib)
    score_torreypines = evaluate_dataset(ind, dataset_torreypines_calib)
    score_trucvert = evaluate_dataset(ind, dataset_trucvert_calib)
    ...
    # scores = [median_score, -std_scores]
    
    return scores
end

function evaluate_forecast_all_individuals(cfg, inds)
    ...
    for i in eachindex(inds)
        ...
        try
            score_grandpop = evaluate_dataset(ind, dataset_grandpopo_full; eval_start_index=length(dataset_grandpopo_calib[1]))
            score_narrabeen = evaluate_dataset(ind, dataset_narrabeen_full; eval_start_index=length(dataset_narrabeen_calib[1]))
            score_duck = evaluate_dataset(ind, dataset_duck_full; eval_start_index=length(dataset_duck_calib[1]))
            score_torreypines = evaluate_dataset(ind, dataset_torreypines_full; eval_start_index=length(dataset_torreypines_calib[1]))
            score_trucvert = evaluate_dataset(ind, dataset_trucvert_full; eval_start_index=length(dataset_trucvert_calib[1]))

            all_scores[i, :] .= [score_grandpop, score_narrabeen, score_duck, score_torreypines, score_trucvert]
        catch e
            println(i, "  ERROR::  ", e)
            score_grandpop = score_narrabeen = score_duck = score_torreypines = score_trucvert = -Inf    
            all_scores[i, :] .= [score_grandpop, score_narrabeen, score_duck, score_torreypines, score_trucvert]
        end
...
function evaluate_calibration_all_individuals(cfg, inds)
    all_scores = Array{Float64}(undef, length(inds), cfg.d_fitness)
    for i in eachindex(inds)
        ind = inds[i]
        if i % 2000 == 0
        ...
        try
            # score_grandpop = evaluate_dataset(ind, dataset_grandpopo_full; eval_end_index=length(dataset_grandpopo_calib[1]))
            # score_narrabeen = evaluate_dataset(ind, dataset_narrabeen_full; eval_end_index=length(dataset_narrabeen_calib[1]))
            # score_duck = evaluate_dataset(ind, dataset_duck_full; eval_end_index=length(dataset_duck_calib[1]))
            # score_torreypines = evaluate_dataset(ind, dataset_torreypines_full; eval_end_index=length(dataset_torreypines_calib[1]))
            # score_trucvert = evaluate_dataset(ind, dataset_trucvert_full; eval_end_index=length(dataset_trucvert_calib[1]))
...
function evaluate_multiple_runs_dataset_v3_single_plots(inds, datasets_calib, datasets_fullseries, site_names; pdf_path="", timeseries_name="", plot_size=(1080, 720), linewidth=1, ind_label="ind")
    ...
    for di in eachindex(site_names)
        ...
        for i in eachindex(inds)
            ...
            if dxdts == nothing || length(dxdts) != length(omega)
                ...
                Plots.plot!(p_spec, window_lengths, shorefor_mielke_scores .- mielke_scores, label="$(ind_label)$(i)", linecolor=colors[i], ylabel="", xlabel="", legend = false, linewidth=linewidth, linealpha=0.9, bottom_margin=8mm)
                Plots.plot!(p1, x_vals, xs[i_pred], xticks=xticks, label="$(ind_label)$(i), λ=$score", linecolor=colors[i], linewidth=linewidth, linealpha=0.9)
        ...
        score_text = "Model, Mielke\n"
        for i in eachindex(scores)
            score_text *= "Ind-$i, $(scores[i])\n"
        end
        # annotate!(p2, text(score_text, :red, :right, 3))
        plot!(p1, size=(680, 180), dpi=200)
        plot!(p_spec, size=(380, 180), dpi=200)
        p3 = Plots.plot(p1, p_spec, layout=Plots. grid(1,2, widths=(0.72, 0.28)), size=(900,180))
        ...
        for i in eachindex(inds)
            ...
            if dxdts == nothing || length(dxdts) != length(omega)
                ...
                ma_x, ma_x̂, window_lengths, mielke_scores = _spectral_eval(eval_target, eval_pred)
                Plots.plot!(p_spec, window_lengths, shorefor_mielke_scores .- mielke_scores, label="$(ind_label)$(i)", linecolor=colors[i], ylabel=L"", xlabel=LaTeXString(L""), legend = false, linewidth=linewidth, linealpha=0.9, bottom_margin=8mm)  # ylabel=L"\Delta λ", xlabel=LaTeXString(L"$Timescale [months]$")
                Plots.plot!(p2, x_vals, eval_pred, xticks=xticks, label="$(ind_label)$(i), λ=$score", linecolor=colors[i], linewidth=linewidth, linealpha=0.9)
            end
        ...
        plot!(p_spec, size=(380, 180), dpi=200)
        p3 = Plots.plot(p2,p_spec, layout=Plots.grid(1,2, widths=(0.72, 0.28)), size=(900,180))
        savefig(p3, "$pdf_path/$timeseries_name-forecast-d$di.pdf")
    end
end
...
function evaluate_multiple_runs_dataset_v3(inds, datasets_calib, datasets_fullseries, site_names; pdf_path="", timeseries_name="", plot_size=(1080, 720), linewidth=1)
    colors=palette(:rainbow, length(inds) + 1)
    fullplot_supertitle = Plots.plot(title=uppercasefirst(replace(timeseries_name, "_" => " ")), framestyle=nothing,showaxis=false,xticks=false,yticks=false,margin=0mm)

    cal_supertitle = Plots.plot(title="Calibration period", framestyle=nothing,showaxis=false,xticks=false,yticks=false,margin=0mm)
    
    for_supertitle = Plots.plot(title="Forecast period", framestyle=nothing,showaxis=false,xticks=false,yticks=false,margin=0mm)
    ...
    for di in eachindex(site_names)
        ...
        p1 = Plots.plot(Dates.format.(num2date.(t_shore), "yyyy-mm"), loc_shore, label="Target", color=:black, legend=:outerbottomright, linewidth=linewidth*2, linealpha=0.7, ylabel=L"X", title=site_name, xrotation = 90, bottom_margin=20mm)
        p_spec = Plots.plot()
        for i in eachindex(inds)
            ind = inds[i]
            CartesianGeneticProgramming.reset!(ind)
            ...
            if dxdts == nothing || length(dxdts) != length(omega)
                ...
                ma_x, ma_x̂, window_lengths, mielke_scores = _spectral_eval(eval_target, eval_pred)
                Plots.plot!(p_spec, window_lengths, mielke_scores, label="ind$i", linecolor=colors[i], ylabel=L"Mielke", xlabel=LaTeXString(L"$Timescale [months]$"), legend = :outerbottomright, linewidth=linewidth, linealpha=0.9, bottom_margin=20mm)
                
                Plots.plot!(p1, Dates.format.(num2date.(dates_waves[i_pred]), "yyyy-mm"), xs[i_pred], label="ind$i, $score", linecolor=colors[i], linewidth=linewidth, linealpha=0.9)
            end
        end
        push!(cal_plots, p1)
        push!(cal_spec_plots, p_spec)
        ...
        phi = ceil(phi / dt_waves)
        local inputs = [omega, P, phi, 2phi, Dir, Hsb, Tp, Sla, rivdis]
        # inputs = [omega, P, phi, 2exphi, Dir, Hsb, Tp]
        
        p2 = Plots.plot(Dates.format.(num2date.(t_shore[calibration_shores_length:end]), "yyyy-mm"), loc_shore[calibration_shores_length:end], label="Target", color=:black, legend=:outerbottomright, linewidth=linewidth*2, linealpha=0.7, ylabel=L"X", title=site_name, xrotation = 90, bottom_margin=20mm)
        p_spec = Plots.plot()
        for i in eachindex(inds)
            ind = inds[i]
            CartesianGeneticProgramming.reset!(ind)
            ...
            if dxdts == nothing || length(dxdts) != length(omega)
                ...
                Plots.plot!(p2, Dates.format.(num2date.(t_shore[calibration_shores_length:end]), "yyyy-mm"), eval_pred, label="ind$i, $score", linecolor=colors[i], linewidth=linewidth, linealpha=0.9)
            end
        end
        push!(for_plots, p2)
        push!(for_spec_plots, p_spec)
    end
    
    p_cal = Plots.plot(cal_plots..., layout=(length(datasets_calib),1), dpi=200, left_margin=10mm)  # size=plot_size, 
    p_spec_cal = Plots.plot(cal_spec_plots..., layout=(length(datasets_calib),1), dpi=200, left_margin=10mm)  # size=plot_size./4, 
    p_side_by_side_cal = Plots.plot(p_cal, p_spec_cal, layout=(1, 2), size=plot_size)
    p_cal_with_title = Plots.plot(cal_supertitle, p_side_by_side_cal, layout=(2, 1))
    
    p_for = Plots.plot(for_plots..., layout=(length(datasets_calib),1), dpi=200, left_margin=10mm)  # size=plot_size, 
    p_spec_for = Plots.plot(for_spec_plots..., layout=(length(datasets_calib),1), dpi=200, left_margin=10mm)  # size=plot_size./4, 
    p_side_by_side_for = Plots.plot(p_for, p_spec_for, layout=(1, 2), size=plot_size)
    p_for_with_title = Plots.plot(for_supertitle, p_side_by_side_for, layout=(2, 1))
    
    # p_side_by_side = plot(p_cal_with_title, p_for_with_title, layout=(1, 2), size=plot_size)
    p_side_by_side_notitles = Plots.plot(p_side_by_side_cal, p_side_by_side_for, layout=(1, 2), size=plot_size, left_margin=20mm)
    
    if pdf_path != ""
        println("saving")
        savefig(p_side_by_side_notitles, "$pdf_path/$timeseries_name.pdf")
    else
        display(Plots.plot(p_side_by_side_notitles))
        readline()
    end
end
fit(i::CGPInd) = evaluate_individual(i)
test_inputs = inputs_from_dataset(dataset_grandpopo_calib)
CartesianGeneticProgramming.mutate(i::CGPInd) = goldman_mutate(cfg, i, test_inputs, evaluate_individual)

function get_fits_and_ranks(population, rank)
    fits_and_ranks = Array{Float64}(undef, cfg.n_population, cfg.d_fitness + 1)  # last index: rank
    for i in eachindex(population)
        ind = population[i]
        for j in collect(1:cfg.d_fitness)
            fits_and_ranks[i, j] = ind.fitness[j]
        end
        fits_and_ranks[i, end] = rank[objectid(ind)]
    end
    fits_and_ranks
end
...
function fastNonDominatedSort(population)
    T=typeof(population[1])
    Fi=Array{T}(undef,0)

    n=Dict(objectid(x)=>0 for x in population)
    S=Dict(objectid(x)=>Array{T}(undef,0) for x in population)
    rank=rank=Dict(objectid(x)=>0 for x in population)
    
    for ind1 in population
        for ind2 in population
            if dominates(ind1,ind2)
                push!(S[objectid(ind1)],ind2)
            elseif dominates(ind2,ind1)
                n[objectid(ind1)]+=1
            end
        end
        if n[objectid(ind1)]==0
            rank[objectid(ind1)]=1
            push!(Fi,ind1)
        end
    end
